...
const TUTORIAL_CHALLENGES = [
    {
        title: '一步取胜',
        shortTitle: '取胜',
        desc: '目标：赢。真实残局里如果红方有一步到底线，必须立刻结束比赛。',
        hint: '点左侧红马，跳到最上方发光格。不要被旁边的蓝子干扰。',
        pieces: [
        ...
        goal: 'win',
        maxMoves: 1,
        success: '果断！看到一步胜时，直接冲线获胜。',
    },
    {
        title: '底线拦截',
        shortTitle: '拦截',
        desc: '目标：阻止蓝方获胜。蓝方下一手就能到底线，红方必须马上处理。',
        ...
        goal: 'stopBlueWin',
        maxMoves: 1,
        success: '拦得好！蓝方已经没有下一手冲线的机会。',
    },
    {
        title: '封住马腿',
        shortTitle: '封腿',
        desc: '目标：阻止蓝方获胜。不一定要吃子，有时占住马腿点就能封死冲线。',
        hint: '蓝马要从下方冲线必须经过它正下方的马腿点。把红马跳到发光格，堵住蓝马腿。',
        pieces: [
            { side: 'player', col: 0, row: 5 },
            { side: 'player', col: 5, row: 4 },
            { side: 'ai', col: 2, row: 5 },
            { side: 'ai', col: 5, row: 1 },
        ],
        target: { col: 2, row: 6 },
        goal: 'stopBlueWin',
        maxMoves: 1,
        success: '好防守！堵住马腿后，蓝方不能立刻到底线。',
    },
    {
        title: '吃光取胜',
        shortTitle: '吃光',
        desc: '目标：赢。蓝方只剩一枚关键子，能吃掉就直接获胜。',
        hint: '红马可以一步跳吃发光格上的最后一枚蓝子。',
        pieces: [
        ...
        goal: 'win',
        maxMoves: 1,
        success: '漂亮！吃光蓝子也是获胜方式。',
    },
    {
        title: '两步冲线',
        shortTitle: '两步胜',
        desc: '目标：赢。蓝方会移动，红方必须在两步内找到最快冲线路线。',
        hint: '第一步跳到中路，第二步跳到最上方底线。不要被蓝方边路移动影响。',
        pieces: [
            { side: 'player', col: 2, row: 4 },
            { side: 'player', col: 5, row: 6 },
            { side: 'ai', col: 0, row: 0 },
    ...
    },
    {
        title: '先赢还是先防',
        shortTitle: '抢胜',
        desc: '目标：赢。蓝方有冲线威胁，但红方也有一步胜；真实对局要先判断谁能立即结束。',
        hint: '不要去防守蓝马。右侧红马可以一步跳到最上方底线，直接获胜。',
        pieces: [
        ...
        goal: 'win',
        maxMoves: 1,
        success: '正确！自己能一步赢时，不需要先防守。',
    },
    {
        title: '识别真威胁',
        shortTitle: '真威胁',
        desc: '目标：阻止蓝方获胜。蓝方有一个真正的一步冲线威胁，也有一个迷惑你的干扰子。',
        hint: '不要追干扰子。把红马跳到发光格，封住真正威胁的马腿。',
        pieces: [
            { side: 'player', col: 5, row: 5 },
            { side: 'player', col: 0, row: 4 },
            { side: 'ai', col: 3, row: 5 },
            { side: 'ai', col: 1, row: 4 },
        ],
        target: { col: 3, row: 6 },
    ...
    },
    {
        title: '最后防线',
        shortTitle: '守住',
        desc: '目标：阻止蓝方获胜。蓝方有一步冲线和一个掩护子，红方必须找出真正的威胁。',
        hint: '掩护子不是重点。吃掉发光格上的蓝马，才能阻止蓝方下一手到底线。',
        pieces: [
            { side: 'player', col: 4, row: 4 },
...
function isTutorialComplete(capturedPiece) {
    ...
    if (challenge.goal === 'win') {
        return gameState.pieces.some(piece => piece.side === 'player' && piece.alive && piece.row === 0)
